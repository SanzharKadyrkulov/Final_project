...
const ProductComments = () => {
  ...
  const sendComment = async (e, id, product) => {
    e.preventDefault();
    if (
      ...
      let newComment = [...product.comments];
  ...
  const deleteComment = async (index, id, product) => {
    let deletedComment = [...product.comments];
    ...
    const data = await axios.delete(
      `http://localhost:8001/api/v1/comments/${id}`
    );
    getProductsData();
    setProduct(productWithoutComment);
  };
  const editComment = async (index, id, product) => {
    handleClose();
    let editedComment = [...product.comments];
    console.log(index);
                ...
                {product ? (
                  product.comments.map((commentix, index) => (
                    <div style={marginOfComment(commentix)}>
                      ...
                      >
                        <p style={{ color: "#e96a1b" }}>{commentix.email}</p>
                        <p
                          style={{
                            fontSize: "12px",
                            color: "gray",
                      ...
                      {whoIsAuthor(commentix) ? (
                        <button
                          onClick={() =>
                            deleteComment(index, commentix.id, product)
                          }
                          style={{
                            transform: "scale(0.7)",
                            color: "#fff",
